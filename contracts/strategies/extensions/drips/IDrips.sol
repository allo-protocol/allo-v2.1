...
interface IDrips {
    ...
    function balances(IERC20 erc20) public view returns (uint256 streamsBalance, uint256 splitsBalance);
    ...
    function receiveStreams(uint256 accountId, IERC20 erc20, uint32 maxCycles) public returns (uint128 receivedAmt);
    ...
    /// on top of what was collectable before.
    /// @return splitAmt The amount split to the account's splits receivers
    function split(uint256 accountId, IERC20 erc20, SplitsReceiver[] calldata currReceivers) public;
    ...
    function collect(uint256 accountId, IERC20 erc20) public onlyDriver(accountId) returns (uint128 amt);
    ...
    function give(uint256 accountId, uint256 receiver, IERC20 erc20, uint128 amt) public onlyDriver(accountId);
  ...
  function balanceAt(
    ...
    function hashStreams(StreamReceiver[] memory receivers) public pure returns (bytes32 streamsHash) {
    ...
    function setSplits(uint256 accountId, SplitsReceiver[] calldata receivers) public onlyDriver(accountId);
    ...
    function hashSplits(SplitsReceiver[] calldata receivers) public pure returns (bytes32 receiversHash);
