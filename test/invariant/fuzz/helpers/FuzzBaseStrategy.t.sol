...
contract FuzzBaseStrategy is BaseStrategy {
    error FuzzBaseStrategy_NoAllocationToDistribute();
    ...
    function _register(
        ...
        bytes memory _data,
        address _sender
    ) internal override returns (address[] memory _recipientIds) {
    ...
    function _distribute(
        ...
        for (uint256 i = 0; i < _recipientIds.length; i++) {
            if (allocated[_recipientIds[i]] == 0)
                revert FuzzBaseStrategy_NoAllocationToDistribute();
