...
contract Pools is Utils, GhostStorage {
    ...
    function _initImplementations(address _allo) internal {
        _strategyImplementations[PoolStrategies.FuzzBaseStrategy] = address(new FuzzBaseStrategy(_allo));
    ...
    function _totalAllocatedInPool(uint256 _poolId) internal returns (uint256 _aggregateAllocation) {
        for (uint256 i; i < _ghost_actors.length; i++) {
            _aggregateAllocation += ghost_allocations[_poolId][_ghost_actors[i]];
        }
    }
